...
- Update the library version linked to the Unity plugin
- Expose new or modified functions added to the updated version of the SDK

## Library

To update the native iOS library linked to the plugin, go to the post processor located in `source/Assets/Plugins/Didomi/Editor/PostProcessor.cs` and find the `RunPodInstall` method. There you need to set `didomiPodVersion` to the version that you want to link to the plugin.
```
        // Didomi Pod Version should be updated here.
        string didomiPodVersion = "1.86.0";
```

The latest native iOS library version can be found [`in our release notes`](https://developers.didomi.io/cmp/mobile-sdk/ios/versions).
